 ...
 *
 * Issue #1 fix: outer loop was `while (ads1256_ring_overflow)` bootstrapped
 * with `ads1256_ring_overflow = true`, which permanently exited after the
 * first clean run (overflow flag false -> loop condition false -> exit).
...
void ads1256_core1_entry(void)
{
    ...
    for (;;)
    {
         ...
         * core0 clears ads1256_core1_run when returning to IDLE, so we
         * must not re-enter the idle wait until it has done so. */
        while (!ads1256_core1_run)
            tight_loop_contents();
